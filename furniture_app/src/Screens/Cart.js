...
const Cart = ({ navigation }) => {
  ...
  const handlePaymentSuccess = async (paymentId) => {
    ...
    // Save the ordered items for further processing (e.g., sending order details to the server, etc.)
    // ...
    ...
    // Save updated cart to AsyncStorage before showing the Toast message
    await saveCartToStorage(cartState.cartItems);
    ...
    // Navigate to the "Order" screen after the Toast message is displayed
    ...
    if (orderedItems.length > 0) {
      // Cart has items, navigate to the "Order" screen
